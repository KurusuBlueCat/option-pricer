...
class DisplaceDiffusion(Black76):
    ...
    def get_bs_sigma(self, F=None, K=None, sigma=None, T=None, A=None, beta=None):

        F, K, sigma, T, beta = self._get_params(F=F, K=K, sigma=sigma,
                                                T=T, beta=beta)

        if A is None:
            A = F

        sigma_bs = self.sigma_black_scholes(F, K, sigma, T, A, beta)
        return sigma_bs
...
class SABR(Black76):
    ...
    def get_bs_sigma(self, F=None, K=None, T=None, alpha=None, beta=None, rho=None,
                     nu=None):
        F, K, alpha, beta, rho, nu, T = self._get_params(F=F, K=K,
